...
if(isset($_GET['code']))
{
	...
	$exam->query = "
	SELECT online_exam_status, online_exam_datetime FROM online_exam_table 
	...
	foreach($result as $row)
	{
		$submit_status = $row['attendance_status'];
...
<?php
if($submit_status == 'Absent' || $submit_status == 'Present')
{
	$exam->data = array(
...
$(document).ready(function(){
	...
	function load_sponsors_details()
	{
		$.ajax({
			url:"user_ajax_action.php",
			method:"POST",
			data:{page:'view_exam', action:'sponsors'},
			success:function(data)
			{
				$('#sponsors_area').html(data);
			}
		})
	}

	load_sponsors_details();
	...
	$(document).on('click', '.submit', function(){
		...
		$.ajax({
			...
			success:function(data)
			{
			alert('Quiz has been submitted succesfully');
			location.reload();	
			}
		})
	});
