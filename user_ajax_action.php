...
if(isset($_POST['page']))
{
	if($_POST['page'] == 'register')
	{
		...
		if($_POST['action'] == 'register')
		{
			...
			(user_email_address, user_password, user_verfication_code, user_name, user_gender, user_class, user_mobile_no, user_image, user_created_on)
			VALUES 
			(:user_email_address, :user_password, :user_verfication_code, :user_name, :user_gender, :user_class, :user_mobile_no, :user_image, :user_created_on)
			";
	...
	if($_POST['page'] == "profile")
	{
		if($_POST['action'] == "profile")
		{
			...
			$exam->query = "
			UPDATE user_table 
			SET user_name = :user_name, user_gender = :user_gender, user_class = :user_class, user_mobile_no = :user_mobile_no, user_image = :user_image 
	...
	if($_POST['page'] == 'index')
	{
		if($_POST['action'] == "fetch_exam")
		{
			...
			foreach($result as $row)
			{
				...
				else
				{
					...
					<tr>
						<td colspan="2" align="center">
							<button type="button" name="enroll_button" id="enroll_button" class="btn btn-warning" data-exam_id="'.$row['online_exam_id'].'" data-online_exam_code="'.$row['online_exam_code'].'">Enroll it</button>
						</td>
					</tr>
					';
				}
		...
		if($_POST['action'] == 'enroll_exam')
		{
			$exam->data = array(
				...
				':online_exam_code'		=>	$_POST['online_exam_code']
			);

			$exam->query = "
			INSERT INTO user_exam_enroll_table 
			(user_id, exam_id, online_exam_code) 
			VALUES (:user_id, :exam_id, :online_exam_code)
			";
	...
	if($_POST["page"] == 'enroll_exam')
	{
		if($_POST['action'] == 'fetch')
		{
			...
			if(isset($_POST["search"]["value"]))
			{
			 	...
			 	$exam->query .= 'OR online_exam_table.user_class LIKE "%'.$_POST["search"]["value"].'%" ';
			...
			foreach($result as $row)
			{
				...
				$sub_array[] = $row["user_class"];
				$sub_array[] = $row["total_question"] . ' Questions';
				$status = '';
				$view_exam = '';
				...
				if($row["attendance_status"] == 'Absent')
				{
					$view_exam = '<a href="view_exam.php?code='.$row["online_exam_code"].'" class="btn btn-info btn-sm" style="background-color:#00BFFF;border:none;color:#ffffff;">Start Exam</a>';
				}
				if($row["attendance_status"] == 'Present')
				{
					$view_exam = '<a href="view_exam.php?code='.$row["online_exam_code"].'" class="btn btn-info btn-sm" style="background-color:#00BFFF;border:none;color:#ffffff;">Continue Exam</a>';
				}
				if($row["attendance_status"] == 'Completed')
				{
					...
					<br>
					<br>
					<button type="button" style="background-color:#00C957;border:none;color:#ffffff;margin-top:-60px;" class="btn btn-warning btn-sm restart" data-exam_id="'.$row["exam_id"].'">Restart</button>';
				}
				$sub_array[] = $status;	
	...
	if($_POST['page'] == 'view_exam')
	{
		if($_POST['action'] == 'load_question')
		{
			...
			foreach($result as $row)
			{
				...
				{
					$is_checked = '';
					if($exam->Get_user_question_option($row['question_id'], $_SESSION['user_id']) == $count)
					{
						$is_checked = 'checked';
					}
					$output .= '
					<div class="col-md-6" style="margin-bottom:32px;">
						<div class="radio">
							<label><h4><input type="radio" name="option_1" class="answer_option"  data-question_id="'.$row["question_id"].'" data-id="'.$count.'"'.$is_checked.'>&nbsp;'.$sub_row["option_title"].'</h4></label>
						</div>
					</div>
					';
				...
				$output .= '
					<br /><br />
				  	<div align="center">
				   		<button type="button" style="background-color:#00BFFF;border:none;color:#ffffff;" name="previous" class="btn btn-info btn-sm previous" id="'.$previous_id.'" '.$if_previous_disable.'>Previous</button>
				   		<button type="button" style="background-color:#00BFFF;border:none;color:#ffffff;" name="next" class="btn btn-warning btn-sm next" id="'.$next_id.'" '.$if_next_disable.'>Next</button>
				  	</div>
					  <br /><br />';
					  ...
					  if($next_id == "")
					  {
						  ...
						  $output .= '
							...
							<div align="center">
								 ...
								 <button type="button" style="background-color:#00C957;border:none;color:#ffffff;margin-top:-60px;" name="submit" class="btn btn-warning btn-sm submit" >Submit Quiz</button>
							</div>
							<br /><br />';
					  }
		...
		if($_POST['action'] == 'question_navigation')
		{
			...
			foreach($result as $row)	
			{
				...
				<div class="col-md-2" style="margin-bottom:24px;">
					<button type="button" style="background-color:#00BFFF;border:none;color:#ffffff;border-radius:50%;" class="btn btn-primary btn-lg question_navigation" data-question_id="'.$row["question_id"].'">'.$count.'</button>
				</div>
				';
				$count++;
			}
			...
			if(isset($_SESSION["user_id"]))
		{
			...
			$result = $exam->query_result();
			foreach($result as $row){
			echo '<img src="upload/'.$row['user_image'].'" class="rounded-circle" style="width:50px;height:50px;border: solid 2px;border-color: #009ACD;"><span style="font-size:28px;">  '.$row['user_name'].'</span>';
			echo '<br>';
			echo'<span style="font-size:20px;">'.$row['user_class'].'</span>';
			}
		}
			
		}
		if($_POST['action'] == 'sponsors')
		{
			$exam->query = "
			SELECT * FROM user_table 
			WHERE user_id = '".$_SESSION["user_id"]."'
